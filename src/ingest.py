...
def match_pdfs_to_sources(sources: dict, pdf_folder: Path) -> tuple[dict, list]:
    """Match PDF files to source citations using fuzzy matching."""
    ...
    for source_num, source in sources.items():
        if source.get("original_filename"):
            ...
            if src.exists():
                ...
                print(f"  {source_num:2d}. [WARNING] File not found: {source['original_filename']}")
        else:
            print(f"  {source_num:2d}. [SKIPPED] No match for: {source['citation'][:40]}...")
...
def validate_setup(sources: dict, pdf_folder: Path) -> list:
    """Validate that all required files exist. Returns list of errors."""
    errors = []

    for source_num, source in sources.items():
        if not source.get("original_filename"):
            errors.append(f"Source {source_num} ({source['citation'][:40]}...): No matching PDF")
        else:
...
def create_sample_excel(output_path: Path) -> None:
    """Create a sample Excel file matching Microsoft Forms format."""
    data = {
        ...
        "rq2_id": ["RQ2"],
        "rq2_text": ["How does screen time affect cognitive development?"],
        "rq2_keywords": ["screen time, cognitive, development"],
        "rq3_id": [""], "rq3_text": [""], "rq3_keywords": [""],
        "rq4_id": [""], "rq4_text": [""], "rq4_keywords": [""],
        "rq5_id": [""], "rq5_text": [""], "rq5_keywords": [""],
        "source_citations": ["Smith et al. (2023) - Meta-analysis\nJones (2024) - Longitudinal study"],
        "pdf_folder_link": [""],
        "context_description": ["Effects of digital technology on learning"],
        "context_population": ["K-12 students"],
        "context_constructs": ["Technology use, learning outcomes"],
...
# Need yaml import at module level
import yaml
